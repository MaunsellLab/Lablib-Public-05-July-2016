...
-(id) init;
{
	if ((self = [super init])) {
		...
		if (digitalOutDevice == nil) {
            [LLSystemUtil runAlertPanelWithMessageText:@"VIDigitalOut" informativeText:
                        @"Can't find data device named \"ITC-18 1\", trying \"ITC-18\" instead." ];
//			NSRunAlertPanel(@"VIDigitalOut",  @"Can't find data device named \"%@\", trying ITC-18 instead.",
//                            @"OK", nil, nil, @"ITC-18 1");
			digitalOutDevice = (LLITC18DataDevice *)[[task dataController] deviceWithName:@"ITC-18"];
			if (digitalOutDevice == nil) {
                [LLSystemUtil runAlertPanelWithMessageText:@"VIDigitalOut" informativeText:
                        @"Can't find data device named \"ITC-18\". (Quitting)" ];
