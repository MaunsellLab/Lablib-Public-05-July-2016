...
int orderedAttentStates [4] [4] = { {0, 2, 1, 3}, {1, 3, 0, 2}, {2, 0, 3, 1}, {3, 1, 2, 0} };

NSString *VIEyeToUseKey = @"VIEyeToUse";

@implementation VIUtilities
...
+ (BOOL)inWindow:(LLEyeWindow *)window;
{
    ...
    switch ([[task defaults] integerForKey:VIEyeToUseKey]) {
        case kUseLeftEye:
        default:
...
+ (void)updateBlockStatus:(BlockStatus *)pBS;
{
	...
	if (pBS->presDoneThisState >= pBS->presPerState) {
		...
		if (++pBS->statesDoneThisBlock >= pBS->statesPerBlock) {
            ...
            // In an attempt to increase differences in attention between states, we jump from IN (FAR) -> FAR (IN)
            // -> IN (FAR) -> FAR (IN). So the only random pick occurs at the start of each new block of kNumStates states.
            
            state = (pBS->attendState < kNumStates / 2) ? pBS->attendState : pBS->attendState - kNumStates / 2;
            for (index = 0; index < kNumStates; index++) {
                if (orderedAttentStates [index] [blocksDoneThisSet - 1] == state) {
                    if (blocksDoneThisSet == kNumStates / 2) { // from IN to OUT or v.v.
                        pBS->attendState = orderedAttentStates [index] [0] + offset;
                    }
                    else {
                        pBS->attendState = orderedAttentStates [index] [blocksDoneThisSet] + offset;
                    }
                    break;
                }
            }
