...
NSString *stimulusMonitorID = @"VICAN Stimulus";

@implementation VIStimuli
...
- (void)doStimSettings;
{
	[stimSettingsWindow makeKeyAndOrderFront:self];
}
...
- (void)dumpStimSequence;
{
    ...
    for (index = 0; index < [stimList count]; index++) {
        string0 = [string0 stringByAppendingString:[VIUtilities stringForStimDescEntry:stimList entryIndex:index]];
...
- (id)init;
{
    if ((self = [super initWithWindowNibName:@"VIStimuli"]) == nil) {
		return nil;
	}
    [self window];
...
// Create and initialize a gabor stimulus
	...
	[cueSpot bindValuesToKeysWithPrefix:@"VICue"];
	targetSpot = [[LLFixTarget alloc] init];
	[targetSpot bindValuesToKeysWithPrefix:@"VITarget"];
	fixSpot = [[LLFixTarget alloc] init];
	[fixSpot bindValuesToKeysWithPrefix:@"VIFix"];
...
- (void)initGabors;
{
	...
	for (g = 0; g < kStimLocs; g++) {
		...
		[gabors[g] bindValuesToKeysWithPrefix:@"VI"];
		...
		[targetGabors[g] removeKeysFromBinding:[NSArray arrayWithObjects:LLGaborAzimuthDegKey, LLGaborElevationDegKey, 
                                          ...
                                          LLGaborSigmaDegKey, LLGaborRadiusDegKey,
                                          LLGaborSpatialPhaseDegKey, LLGaborTemporalFreqHzKey, nil]];
		[targetGabors[g] bindValuesToKeysWithPrefix:@"VITarget"];
																	...
																	stimTypes:(short *)types;
{
	long stim;
	StimDesc stimDesc;
    float relativeDistContrast = [[task defaults] floatForKey:VIRelDistContrastKey];
    ...
    stimDesc.targetOuterRadius = [targetSpot outerRadiusDeg];
	for (stim = 0; stim < kStimLocs; stim++) {
        ...
        if (!trainingMode) {
            ...
            if (stim == stimDesc.attendLoc) {
                stimDesc.contrasts[stim] = 1;
            }
            else {
                stimDesc.contrasts[stim] = relativeDistContrast * trial.targetAlphaChange;
            }
        }
	}
...
- (long)jitteredInterstimFrames;
{
	...
	long interJitterTauMS = [[task defaults] integerForKey:VIInterstimJitterTauMSKey];
	
    jitteredFrames = MAX(1, [[task defaults] integerForKey:VIInterstimMSKey] / 1000.0 * frameRateHz);
...
- (void)makeStimList:(TrialDesc *)pTrial;
{
	...
	stimRateHz = 1000.0 / ([[task defaults] integerForKey:VIStimDurationMSKey] + 
					[[task defaults] integerForKey:VIInterstimMSKey]);
...
// set targetIndex to stimListLength so we can use it as a limit in either case.
	...
	for (stim = 0; stim < kStimLocs; stim++) {
		targetIndices[stim] = trial.targetIndex + 1;
		if (stim == trial.targetPos) {
...
// position
	...
	frontPadStim = MAX(1, MIN(targetIndex, ceil([[task defaults] integerForKey:VIStimLeadMSKey] 
                                                / 1000.0 * stimRateHz)));
...
// If this is not a catch trial, load the target stimulus, using a randomly selected base
	...
	if (!trial.catchTrial && targetIndex < stimListLength) {	
		for (stim = 0; stim < kStimLocs; stim++) {
			listTypes[stim] = (stim == trial.targetPos) ? kTargetStim : kPadding;
		}
		[self insertStimSettingsAtIndex:-1 trial:pTrial listTypes:listTypes stimTypes:[self randomVisibleIndicesTarget]];
	}
	
// Load the trailing stimuli.  These are just pulled at random and not tallied or shuffled.

    for (t = 0; t < kStimLocs; t++) {
        if ([[task defaults] boolForKey:VITrainingModeKey] && t == trial.targetPos) {
...
//
//	for (stim = 0; stim < kStimLocs; stim++) {
//        if (stim == trial.targetPos || targetIndices[stim] >= trial.numStim) {
//            continue;
//        }
...
// will appear there. 
	...
	stimDurFrames = [[task defaults] integerForKey:VIStimDurationMSKey] / 
					1000.0 * frameRateHz;
	stimJitterPC = [[task defaults] integerForKey:VIStimJitterPCKey];
	stimJitterFrames = stimDurFrames / 100.0 * stimJitterPC;
	stimDurBase = stimDurFrames - stimJitterFrames;
	interDurFrames = [[task defaults] integerForKey:VIInterstimMSKey] / 1000.0 * frameRateHz;
	interJitterTauMS = [[task defaults] integerForKey:VIInterstimJitterTauMSKey];
...
// Set the stimulus directions.  The target directions will be handled when the stimuli are presented
        ...
        for (s = 0; s < kStimLocs; s++)  {
            switch (stimDesc.stimTypes[s]) {
                case kPrefStim:
                case kNoStim:
                    stimDesc.directionsDeg[s] = [[task defaults] floatForKey:VIDirectionDegKey];
                    break;
                case kInter0Stim:
                    stimDesc.directionsDeg[s] = [[task defaults] floatForKey:VIDirectionDegKey] + 45.0;
                   break;
                case kNullStim:
                    stimDesc.directionsDeg[s] = [[task defaults] floatForKey:VIDirectionDegKey] + 90.0;
                   break;
                case kInter1Stim:
                    stimDesc.directionsDeg[s] = [[task defaults] floatForKey:VIDirectionDegKey] + 135.0;
                    break;
                default:
            ...
            if (stimDesc.listTypes[s] == kTargetStim) {
                stimDesc.centerContrast[s] = MAX(0.0, stimDesc.contrasts[s] - trial.targetAlphaChange);
            }
            else {
...
// Set up the gabors
	...
	for (g = 0; g < kStimLocs; g++) {
		[gabors[g] setSpatialFreqCPD:[[task defaults] floatForKey:VISpatialFreqCPDKey]];
        [gabors[g] setSpatialPhaseDeg:0.0];
		[gabors[g] setTemporalFreqHz:0.0];
        [gabors[g] setTemporalPhaseDeg:0.0];						// same odd phase
		[gabors[g] setRadiusDeg:[[task defaults] floatForKey:VIRadiusDegKey]];
		//[gabors[g] setSigmaDeg:[[task defaults] floatForKey:VISigmaDegKey]];
        
        // Set Gabor and target to a size based on a given eccentricity
        // This is easy for Steve and Alex while they train Tintin but it may be useful anyway:
        
        [gabors[g] setSigmaDeg:(float) 0.04 * ([[task defaults] floatForKey:VIEccentricityDegKey] - 5.0) + 0.31];
        [targetSpot setOuterRadiusDeg:(float) 0.04 * ([[task defaults] floatForKey:VIEccentricityDegKey] - 5.0) + 0.29];
        
		[targetGabors[g] setSpatialFreqCPD:[[task defaults] floatForKey:VISpatialFreqCPDKey]];
		...
		[targetGabors[g] setRadiusDeg:[[task defaults] floatForKey:VISigmaDegKey]];
		[targetGabors[g] setSigmaDeg:[[task defaults] floatForKey:VISigmaDegKey]];
	}
	stimIndex = 0;
...
//                    }
		...
		if (trialFrame == stimDesc.stimOnFrame) {
			...
			[[stimList objectAtIndex:stimIndex] getValue:&stimDesc];
			[self loadGaborsWithStimDesc:&stimDesc];
			if (![[task defaults] boolForKey:VITrainingModeKey]) {
				gaborFrame = 0;
			}
		}
...
// Return a random stimulus types that will be visible at the target position (and diametrically opposite of it) but not necessarily at the nontarget position (and opposite of it)

- (short *)randomVisibleIndicesTarget;
{
	static short indices[kStimLocs];
	
    if (trial.targetPos == 0 || trial.targetPos == 2) {
        indices[kGabor0] = indices[kGaborFar0] = kNullStim + (rand() % 2);
        indices[kGabor1] = indices[kGaborFar1] = kNoStim + (rand() % 3);
    }
    else if (trial.targetPos == 1 || trial.targetPos == 3) {
        indices[kGabor0] = indices[kGaborFar0] = kNoStim + (rand() % 3);
        indices[kGabor1] = indices[kGaborFar1] = kNullStim + (rand() % 2);
    }
...
// Set high contrast cue at the attended location and low contrast cues at distractor locations if wrong or distracted (see VIEndTrialState for the exact conditions for invoking this method)
