...
NSString *VISummaryWindowBrokeKey = @"VISummaryWindowBroke";
NSString *VISummaryWindowComputerKey = @"VISummaryWindowComputer";
NSString *VISummaryWindowCorrectKey = @"VISummaryWindowCorrect";
NSString *VISummaryWindowDateKey = @"VISummaryWindowDate";
NSString *VISummaryWindowDistractedKey = @"VISummaryWindowDistracted";
NSString *VISummaryWindowEarlyKey = @"VISummaryWindowEarly";
NSString *VISummaryWindowFailedKey = @"VISummaryWindowFailed";
NSString *VISummaryWindowIgnoredKey = @"VISummaryWindowIgnored";
NSString *VISummaryWindowTotalKey = @"VISummaryWindowTotal";
NSString *VISummaryWindowWrongKey = @"VISummaryWindowWrong";
...
@implementation VISummaryController

- (void)dealloc {

	[[task defaults] setFloat:[NSDate timeIntervalSinceReferenceDate] forKey:VISummaryWindowDateKey];
	[[task defaults] setInteger:dayEOTs[kEOTBroke] forKey:VISummaryWindowBrokeKey];
	[[task defaults] setInteger:dayEOTs[kEOTCorrect] forKey:VISummaryWindowCorrectKey];
	[[task defaults] setInteger:dayEOTs[kEOTFailed] forKey:VISummaryWindowFailedKey];
	[[task defaults] setInteger:dayEOTs[kEOTIgnored] forKey:VISummaryWindowIgnoredKey];
	[[task defaults] setInteger:dayEOTs[kEOTWrong] forKey:VISummaryWindowWrongKey];
	[[task defaults] setInteger:dayEOTs[kEOTIgnored + 1] forKey:VISummaryWindowEarlyKey];
	[[task defaults] setInteger:dayEOTs[kEOTIgnored + 1] forKey:VISummaryWindowDistractedKey];
	[[task defaults] setInteger:dayEOTTotal forKey:VISummaryWindowTotalKey];
	[[task defaults] setInteger:dayComputer forKey:VISummaryWindowComputerKey];
    [fontAttr release];
...
- (id)init;
{
    ...
    if ((self = [super initWithWindowNibName:@"VISummaryController" defaults:[task defaults]]) != nil) {
		...
		timeStored = [[task defaults] floatForKey:VISummaryWindowDateKey];
		timeNow = [NSDate timeIntervalSinceReferenceDate];
		if (timeNow - timeStored < 12 * 60 * 60) {			// Less than 12 h old?
			dayEOTs[kEOTBroke] = [[task defaults] integerForKey:VISummaryWindowBrokeKey];
			dayEOTs[kEOTCorrect] = [[task defaults] integerForKey:VISummaryWindowCorrectKey];
			dayEOTs[kEOTFailed] = [[task defaults] integerForKey:VISummaryWindowFailedKey];
			dayEOTs[kEOTIgnored] = [[task defaults] integerForKey:VISummaryWindowIgnoredKey];
			dayEOTs[kEOTWrong] = [[task defaults] integerForKey:VISummaryWindowWrongKey];
			dayEOTs[kEOTIgnored + 1] = [[task defaults] integerForKey:VISummaryWindowEarlyKey];
			dayEOTs[kEOTIgnored + 2] = [[task defaults] integerForKey:VISummaryWindowDistractedKey];
			dayEOTTotal = [[task defaults] integerForKey:VISummaryWindowTotalKey];
			dayComputer = [[task defaults] integerForKey:VISummaryWindowComputerKey];
		}
    }
    return self;
}
...
// Do nothing if the data buffers have nothing in them
    ...
    switch (row) {
        case 0:
            string = [NSString stringWithFormat:@"%@ (alpha %.2f %@)", stateLabels[trial.attendState],
						  trial.targetAlphaChange,(trial.instructTrial) ? @", Instruction" : @""];
			break;
        case 1:
			string = @"";
            break;
...
- (void)extendedEOT:(NSData *)eventData eventTime:(NSNumber *)eventTime;
{
	...
	[eventData getBytes:&extEotCode length:sizeof(long)];
