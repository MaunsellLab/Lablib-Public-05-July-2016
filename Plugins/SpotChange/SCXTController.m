...
NSString *SCXTAutosaveKey = @"SCXTAutosave";

@implementation SCXTController
...
- (id)init {
    ...
    if ((self = [super initWithWindowNibName:@"SCXTController"]) != nil) {
 		[self setShouldCascadeWindows:NO];
        [self setWindowFrameAutosaveName:SCXTAutosaveKey];
...
// setting the maximum zoom size when the scale changes.
	...
	[[self window] setFrameUsingName:SCXTAutosaveKey];			// Needed when opened a second time
...
- (void)stimulus:(NSData *)eventData eventTime:(NSNumber *)eventTime;
{
	...
	if (stimDesc.stim0Type == kTargetStim || stimDesc.stim1Type == kTargetStim) {
		[xtView eventName:[NSString stringWithFormat:@"%@ %.2f",
                    (trial.validTrial) ? @"Valid" : @"Invalid", trial.orientationChangeDeg] eventTime:eventTime];
	}
}
