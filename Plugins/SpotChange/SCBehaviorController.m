...
@implementation SCBehaviorController
...
- (void)checkParams;
{
	...
	[reactPlot setXAxisLabel:@"Alpha Change"];
	[perfPlot setPoints:pCurrent->changes];
	[perfPlot setXAxisLabel:@"Alpha Change"];
	[self positionPlots];
...
- (id) init;
{
    if ((self = [super initWithWindowNibName:@"SCBehaviorController" defaults:[task defaults]]) != nil) {
    }
    return self;
}
...
- (void)makeLabels;		// make X labels for alpha
{
    long index, levels;
	NSString *string;
...
- (void) windowDidLoad {
    ...
    NSColor *leftValidColor = [NSColor colorWithDeviceRed:0.00 green:0.50 blue:0.00 alpha:1.0];
    NSColor *leftInvalidColor = [NSColor colorWithDeviceRed:0.50 green:0.85 blue:0.50 alpha:1.0];
    NSColor *rightValidColor = [NSColor colorWithDeviceRed:0.50 green:0.00 blue:0.50 alpha:1.0];
    NSColor *rightInvalidColor = [NSColor colorWithDeviceRed:1.0 green:0.00 blue:1.0 alpha:1.0];
...
// want to see those values.  They are mirror image to the correct data
    ...
    for (p = 0; p < kEOTs; p++) {
		...
		switch (p) {
            case kEOTLeftValid:
                [perfPlot addPlot:performance[p] plotColor:leftValidColor];
                break;
            case kEOTLeftInvalid:
                [perfPlot addPlot:performance[p] plotColor:leftInvalidColor];
                break;
            case kEOTRightValid:
                [perfPlot addPlot:performance[p] plotColor:rightValidColor];
                break;
            case kEOTRightInvalid:
                [perfPlot addPlot:performance[p] plotColor:rightInvalidColor];
                break;
            default:
...
// Process reaction time on correct trials only
	...
	if ((eotCode == kMyEOTCorrect) & (saccadeStartTimeMS > 0)) {
		reactTimeMS = saccadeStartTimeMS - targetOnTimeMS;	// defined as time from target onset to the end of the saccade
...
// because they occur before a change value is defined.   
    ...
    validIndex = (trial.attendLoc == kAttend0) ? kEOTLeftValid : kEOTRightValid;
    invalidIndex = (trial.attendLoc == kAttend0) ? kEOTLeftInvalid : kEOTRightInvalid;
	switch (eotCode) {
        case kEOTCorrect:
			[[performance[(trial.validTrial) ? validIndex : invalidIndex] objectAtIndex:dirChangeIndex] addValue:1];
            break;
        case kEOTFailed:
			[[performance[(trial.validTrial) ? validIndex : invalidIndex] objectAtIndex:dirChangeIndex] addValue:0];
            break;
        case kEOTBroke:
            brokeValue = 1;
            break;
