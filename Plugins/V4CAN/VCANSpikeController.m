...
NSString *VCANHistogramSetIndexKey = @"VCANHistogramSetIndex";

@implementation VCANSpikeController
...
- (void)dealloc;
{
    ...
    [[NSUserDefaultsController sharedUserDefaultsController] removeObserver:self 
                                forKeyPath:[NSString stringWithFormat:@"values.%@", VCANHistogramSetIndexKey]];
...
- (id) init;
{
    (self = [super initWithWindowNibName:@"VCANSpikeController" defaults:[task defaults]]);
    return self;
}
...
- (void)loadHistograms;
{
    ...
    BOOL doInIn = ([[task defaults] integerForKey:VCANHistogramSetIndexKey] == 0);
                                                                            ...
                                                                            context:(void *)context;
{
	...
	if ([key isEqualTo:VCANHistogramSetIndexKey]) {
        [self loadHistograms];
    }
}
