...
@implementation VCANPreCueState

- (void)stateAction {

	long preCueMS = [[task defaults] integerForKey:VCANPrecueMSKey];
	
	if (preCueMS > 0) {
		preCueMS = preCueMS * 0.5 + (rand() % preCueMS);
	}
	if ([[task defaults] boolForKey:VCANFixateKey]) {				// fixation required && fixated
