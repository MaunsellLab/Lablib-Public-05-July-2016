...
NSString *VCANXTAutosaveKey = @"VCANXTAutosave";

@implementation VCANXTController
...
- (id)init {
    ...
    if ((self = [super initWithWindowNibName:@"VCANXTController"]) != nil) {
 		[self setShouldCascadeWindows:NO];
        [self setWindowFrameAutosaveName:VCANXTAutosaveKey];
...
// setting the maximum zoom size when the scale changes.
	...
	[[self window] setFrameUsingName:VCANXTAutosaveKey];			// Needed when opened a second time
...
- (BOOL) windowShouldClose:(NSNotification *)aNotification {
    ...
    [[task defaults] setObject:[NSNumber numberWithBool:NO] forKey:trialWindowVisibleKey];
    [NSApp addWindowsItem:[self window] title:[[self window] title] filename:NO];
    return NO;
}
...
{
    NSString *labelStrings[kRFStimTypes] = {@"-", @"N", @"P"};
	StimDesc stimDesc;
	
	[eventData getBytes:&stimDesc];
	[xtView stimulusBarColor:[NSColor grayColor] eventTime:eventTime];
	if ((stimDesc.attendLoc == 0 && stimDesc.listTypes[kGabor0] == kTargetStim) ||
			(stimDesc.attendLoc == 1 && stimDesc.listTypes[kGabor1] == kTargetStim)) {
		[xtView eventName:@"Target" eventTime:eventTime];
	}
	else {
		[xtView eventName:[NSString stringWithFormat:@"%@%@",labelStrings[stimDesc.stimTypes[0]], 
                           labelStrings[stimDesc.stimTypes[1]]] eventTime:eventTime];
	}
}
