...
@implementation VCANBehaviorController
...
- (id) init;
{
	return [super initWithWindowNibName:@"VCANBehaviorController" defaults:[task defaults]];
}
...
- (void)staircaseResult:(NSData *)eventData eventTime:(NSNumber *)eventTime;
{
	...
	[eventData getBytes:&staircaseResult];
...
- (void)trial:(NSData *)eventData eventTime:(NSNumber *)eventTime;
{
    ...
    targetIndex = trial.targetIndices[trial.attendLoc];
    for (locIndex = 0, perfPlotIndex = kNoDistractorPlot; locIndex < kStimLocs; locIndex++) {
        if (locIndex == trial.attendLoc) {
            continue;
        }
        if (trial.targetIndices[locIndex] < targetIndex) {
            perfPlotIndex = kDistractorPlot;
            break;
        }
    }
    [perfTimePlot[kNoDistractorPlot] setHighlightXRangeFrom:0 to:0];
...
- (void)extendedEOT:(NSData *)eventData eventTime:(NSNumber *)eventTime;
{
    ...
    targetIndex = (trial.catchTrial) ? maxTargetIndex - 1 : trial.targetIndices[trial.attendLoc] - 1;
