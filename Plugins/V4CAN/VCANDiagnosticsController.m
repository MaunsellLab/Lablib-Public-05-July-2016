...
#define VCANDiagnosticWindowVisibleKey      @"VCANDiagnosticWindowVisible"
#define VCANDiagnosticsAutosaveKey          @"VCANDiagnosticsAutosave"
#define VCANDiagnoisticsWindowVisibleKey    @"VCANDiagnoisticsWindowVisible"

@implementation VCANDiagnosticsController
...
- (id)init;
{
    if ((self = [super initWithWindowNibName:@"VCANDiagnosticsController"]) != nil) {
...
- (id)stimListTableObject:(NSTableColumn *)tableColumn row:(int)row;
{
    ...
    string = [VCANUtilities stringForStimDescEntry:stimList entryIndex:entryNum];
...
{
	[[task defaults] setObject:[NSNumber numberWithBool:YES] forKey:VCANDiagnosticWindowVisibleKey];
}
...
- (void)windowDidLoad;
{
	[[self window] setFrameUsingName:VCANDiagnosticsAutosaveKey];			// Needed when opened a second time
    if ([[task defaults] boolForKey:VCANDiagnosticWindowVisibleKey]) {
...
- (BOOL)windowShouldClose:(NSNotification *)aNotification;
{
    ...
    [[task defaults] setObject:[NSNumber numberWithBool:NO] forKey:VCANDiagnosticWindowVisibleKey];
    [NSApp addWindowsItem:[self window] title:[[self window] title] filename:NO];
    return NO;
}
